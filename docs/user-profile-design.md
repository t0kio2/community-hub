...
`accounts` は認証情報であり、user / tenant / admin の全主体が共有する。ユーザー向けプロフィールは認証情報ではなく、サービス利用者である `users` の属性として扱う。
...
- `user_profiles.user_id` で `users` に 1:1 で紐づける。
- `Account` からプロフィールを直接参照しない。
- `User has_one :user_profile` を追加する。
- `UserProfile belongs_to :user` とする。
...
- DB テーブル名: `user_profiles`
- Rails モデル名: `UserProfile`
- Rails 関連名: `User has_one :user_profile`
- Rails controller 名: `Api::V1::User::UserProfilesController`
- フロント画面 URL: `/profile`
- API パス: `/api/v1/user/profile`
- API の JSON キー: `user_profile`

画面名と API パスはユーザーに見える概念として単数の `profile` を使う。DB、Rails モデル、関連名、JSON キーは user 専用であることが分かるように `user_profile` / `user_profiles` に統一する。
...
- 未作成の場合は空フォームを表示し、保存時に user_profile を作成する。
...
user_profile 未作成の場合は `200 OK` で `{"user_profile": null}` を返す。フロントが未作成状態と通信エラーを区別できるようにするため、未作成を `404` にはしない。
...
user_profile が存在しない場合は作成し、存在する場合は更新する。レスポンスは取得 API と同じ形にする。
...
- `backend/config/routes.rb` に `GET /api/v1/user/profile` と `PUT /api/v1/user/profile` を追加する。
- `Api::V1::User::UserProfilesController` は既存の `Api::V1::User::BaseController` を継承し、JWT 認証済みの `current_user` のみを扱う。
- user API の `current_user` は `current_user_account.user` を返す。新規登録時は `User` を同一トランザクションで作成し、user API 内で暗黙に `User` を作らない。
- `show` は `current_user.user_profile` を JSON キー `user_profile` で返し、未作成時は `null` を返す。
- `update` は `current_user.user_profile || current_user.build_user_profile` に許可済みパラメータを適用し、保存成功時は `show` と同じレスポンス形にする。
- 保存失敗時は `422 Unprocessable Entity` と `errors: string[]` を返す。
- コントローラテストでは未認証、未作成取得、作成、更新、バリデーションエラーを確認する。
...
- `backend/app/controllers/api/v1/user/user_profiles_controller.rb`
...
- 新規登録時に `accounts` と対応する `users` が作成されること。
- dashboard の「プロフィール」から `/profile` に遷移できること。
- 未ログイン状態で `/profile` を開くと `/auth/login` に戻ること。
- user_profile 未作成時に空フォームが表示されること。
- user_profile 作成済み時に API の値がフォームへ反映されること。
