現状は「宿泊施設の基本構成」はかなり揃っていますが、完成とはまだ言えません。
  ...
  先に直すべき問題があります。Stay Listingフォームの宿泊設定は listing[stay_listing][...] で送信されていますが、StaysController がそのパラメータを許可・代入していません。そのため、チェックイン時
  刻、予約受付期間、ハウスルールなどが保存されない状態です。ここを修正してテストを追加するのが最優先です。
