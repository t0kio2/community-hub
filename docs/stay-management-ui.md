...
施設ごとに1つの販売カレンダーを持ち、`/tenant/stays/:stay_id/sales_calendar`で月単位に表示する。画面名とURLは業務上の役割を表す`Sales Calendar`とし、保存先テーブル名をURLへ露出しない。

- 行はRoom Type配下の有効なRoom Type Rate、列は対象月の宿泊日とする。
- 通常セルは`stay_room_type_rates.price_per_night_amount`を表示する。
- `stay_room_type_rate_daily_prices`がある日は日別料金を優先し、「個別」表示で基本料金と区別する。
- Room Typeごとに販売上限行を設け、レコードなしは「上限なし」、`sales_limit = 0`は「販売停止」、正数は設定数を表示する。
- 前月・翌月および年月指定で表示月を切り替える。無効な年月は当月へ戻す。
- 料金セルから日別料金を設定し、「基本料金へ戻す」で日別料金レコードを削除する。
- 販売上限セルから0以上の上限を設定し、0は販売停止、「上限なしへ戻す」で販売制御レコードを削除する。
- セル選択後は同じ月の編集パネルを表示し、保存後も対象月を維持する。
- 期間一括設定は、単日の更新操作が安定した後に別エンドポイントとして追加する。
- Room Type Rateまたは日別設定の取得は現在のStay Listingを起点にし、別施設のデータを表示しない。
