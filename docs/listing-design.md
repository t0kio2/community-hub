...
- user 側は web から API を呼び出して公開済み listing を表示する。
...
- web 側は `/app/listings` で一覧、`/app/listings/$listingId` で詳細を表示する。
...
5. web 側で公開 listing 一覧・詳細画面を実装する。
...
- web 側は一覧・詳細のローディング、成功、失敗表示をテストまたは手動確認する。
...
```sh
docker compose run --rm -e RAILS_ENV=test -e DATABASE_URL=postgres://app:app@db:5432/app_test api bin/rails test test/models/listing_test.rb
```
